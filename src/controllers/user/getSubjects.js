...
export default async function getSubjects (req, res) {
  try {
    ...
    const grupedSubjects = sqlRequest.map(subject => {
      const {
        idSubject,
        subjectName,
        idSeccion,
    ...
    mergeObjectsByIdSubject(grupedSubjects)
...
function mergeObjectsByIdSubject (arr) {
  ...
  for (const obj of arr) {
    if (mergedObjects[obj.idSubject]) {
      // Si el objeto ya existe en mergedObjects, se fusiona con el objeto existente
      mergedObjects[obj.idSubject].evalPlan = {
        ...mergedObjects[obj.idSubject].evalPlan,
        ...obj.evalPlan
      }
    } else {
      // Si el objeto no existe en mergedObjects, se agrega como un nuevo objeto
      mergedObjects[obj.idSubject] = obj
    }
  }

  // Convertir el objeto de fusión en un arreglo
